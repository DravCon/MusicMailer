#!/usr/bin/env python3
"""
Music Mailer — Daily music newsletter generated by Claude via AWS Bedrock.
...
# ---------------------------------------------------------------------------
# Step 1: Generate newsletter content via Claude (Bedrock)
# ---------------------------------------------------------------------------
...
def generate_newsletter_content() -> dict:
    """Call Claude via Bedrock to compile the newsletter."""
    client = anthropic.AnthropicBedrock(
        aws_region=BEDROCK_REGION,
        aws_api_key=BEDROCK_API_KEY,
    )
    today  = datetime.now().strftime("%A, %B %d, %Y")

    prompt = f"""Today is {today}. Compile a daily music digest with these five sections.
...
{{
  ...
  "hifi_corner": [
    ...
    log.info("Calling Claude via Bedrock…")
...
def _check_config() -> list[str]:
    required = ["BEDROCK_API_KEY", "SMTP_USER", "SMTP_PASSWORD", "RECIPIENT_EMAIL"]
...
def main() -> int:
    ...
    try:
        log.info("Generating newsletter via Claude (Bedrock)…")
